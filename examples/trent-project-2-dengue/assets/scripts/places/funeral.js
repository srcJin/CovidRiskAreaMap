let funeral = document
  .querySelector(".funeral")
...
function getFuneral() {
  ...
  xhr.open(
    "GET",
    "https://cors-anywhere.herokuapp.com/https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=1.3497222222,103.9544444444&radius=4000&type=funeral_home&key=AIzaSyAQOzXrUwtwRVkzSyWzeRdxfpiPe7kBliU",
    true
  );
  ...
  xhr.onload = function () {
    if (this.status === 200) {
      ...
      loadFuneralMarkers(response);
      loadFuneralTable(response);
    } else {
      alert(
...
function loadFuneralMarkers(googleData) {
  ...
  for (let i = 0; i < googleObject.length; i++) {
    ...
    let tableHeadIcon = '<i class="fas fa-heart-broken"></i>';
...
function loadFuneralTable(googleData) {
