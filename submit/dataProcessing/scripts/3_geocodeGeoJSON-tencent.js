...
async function geocodeAxios(address) {
  ...
  let recordRaw = fs.readFileSync("recordRaw_tencent.json");
  ...
  if (hasRecord) {
    ...
    try {
      const response = await axios.get(
        ...
        &key=JMPBZ-ZXTLD-JHU4Y-PFWMK-WCEM6-JQFBU`
      );
      ...
      if (response.data.status == 0) {
        ...
        let coordinate = [response.data.result.location.lng,response.data.result.location.lat];
        pair.coordinates.push(coordinate);

        dict[address] = coordinate

        fs.writeFileSync("recordRaw_tencent.json", JSON.stringify(raw));
