...
async function plotRiskPoints(date) {
  ...
  // Define the custom marker icon
  const highRiskPoints = await axios.get("data/pointsData/" + date + "_h.geojson");
  const middleRiskPoints = await axios.get("data/pointsData/" + date + "_m.geojson");
...
async function plotCase(date) {
  ...
  let provinceGeoJSON = await axios.get("data/provinceData/" + date + ".geojson");
