...
let firstDrawPoints = 0;
let firstDrawCases = 0;
...
async function plotCase(date) {
  ...
  /* start draw cases on map */
  geojson = L.geoJson(provinceGeoJSON.data, {
    style,
    onEachFeature,
  }).addTo(map);
  /* end draw cases on map */
  firstDrawCases = 1

  } else {
    console.log("plotCases > 1");
    // skip draw 
  /* start draw cases on map */
  ...
  // map.addLayer(caseConfirmed);
  // map.addLayer(caseDeath);
  // map.addLayer(caseRecovered);


  // const caseDeath = L.geoJson(provinceGeoJSON.data, {
  //   style,
  //   onEachFeature,
  // }).addTo(map);
  ...
  // supporting functions to draw cases
